...
struct GrammarTextEditor: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UITextView {
        ...
        // Listen for spell check toggle
        NotificationCenter.default.addObserver(
            forName: .init("ToggleSpellCheck"),
            object: nil,
            queue: .main
        ) { _ in
            // Show spell checking UI
            let range = textView.selectedRange
            textView.selectedRange = NSRange(location: 0, length: textView.text.count)
            textView.selectedRange = range
            
            // Toggle spell checking type
            textView.spellCheckingType = textView.spellCheckingType == .yes ? .no : .yes
            textView.autocorrectionType = textView.autocorrectionType == .yes ? .no : .yes
        }
